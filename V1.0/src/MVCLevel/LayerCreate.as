...
public class LayerCreate extends Sprite
{
    ...
    public function LayerCreate(loaderInfo:LoaderInfo,type : int = 0)
    {

        var bitm : BitmapData = new BitmapData(loaderInfo.width,loaderInfo.height,true,0x00000000);
        bitm.draw(loaderInfo.loader);
        this.image = new Bitmap(bitm);
        addChild(this.image);

        if (type == 1)
        {
            movie =(loaderInfo.loader.content as MovieClip);
            addChild(movie);
